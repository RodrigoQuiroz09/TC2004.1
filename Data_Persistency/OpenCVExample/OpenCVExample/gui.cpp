...
int main() {
	...
	while (true)
	{
		...
		std::tuple<Subject, bool> userIdenti;
		userIdenti = frs.personVerification(userCapture, userKey);

		//Cuando el usuario es aceptado:
		if (std::get<1>(userIdenti)) {
			cvui::printf(frame, 300, 75, 0.4, 0xF2F7FB, "Datos del Usuario:");
			cvui::printf(frame, 300, 100, 0.4, 0xF2F7FB, "Nombre: %s", std::get<0>(userIdenti).name);
